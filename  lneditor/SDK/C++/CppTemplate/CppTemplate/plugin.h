...
extern "C" {
	MRESULT __stdcall _GetText(LPFILE_INFO lpFileInfo, LPDWORD lpdwReturnInfo);
...
//Fuctions to export
extern "C" {
	void	__stdcall	InitInfo(LPMEL_INFO);
	int		__stdcall	Match(LPCWSTR);
	void	__stdcall	PreProc(LPPRE_DATA);

	MRESULT	__stdcall	GetText(LPFILE_INFO, LPDWORD);
	MRESULT	__stdcall	ModifyLine(LPFILE_INFO, DWORD);
	MRESULT	__stdcall	SaveText(LPFILE_INFO);

	//The next functions are optional
	MRESULT	__stdcall	SetLine(LPCWSTR, LPSEL_RANGE);
	MRESULT	__stdcall	RetLine(LPCWSTR);

	MRESULT	__stdcall	Release(LPFILE_INFO);
	MRESULT	__stdcall	GetStr(LPFILE_INFO, LPDWORD, LPSTREAM_ENTRY);
